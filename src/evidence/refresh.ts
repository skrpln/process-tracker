// Process Tracker — what a changed evidence note does to the tables on screen.
...
export interface RefreshPlan {
	...
	clear: CellRef[];
	/** The cell the note claims now; `null` when it is no longer evidence. */
 ...
 * `data-evidence`, so the caller reads them straight out of the table instead of the plugin
 * keeping a copy of the vault in memory. A note that moved to another day — or stopped being
 * evidence at all — leaves those cells behind; a note that still claims its cell keeps it and
 * only changes its look.
 *
 * Emptying a cell is a statement about this note, not a verdict on the day: a second note may
 * claim the same day, and the table picks that up at its next render ([[evidence]]).
 */
export function planRefresh(evidence: Evidence | null, claimed: CellRef[]): RefreshPlan {
	const paint: CellPaint | null =
		evidence === null
			? null
			: {
					cell: { trackPath: evidence.trackPath, date: evidence.date },
					state: cellState(evidence),
